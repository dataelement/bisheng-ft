...
class SFTManage(object):
    ...
    def delete_job(self, model_name: str):
        ...
        if pub_model_name is None:
            logger.warning(f'[{model_name}] not published')
            return
    ...
    def publish_job(self, model_name: str):
        ...
        if os.path.exists(publish_model_dir):
            logger.warning(f'model already exists, [{publish_model_dir}]')
        else:
            shutil.move(model_output_dir, publish_model_dir)
        # 存储发布的模型名称
